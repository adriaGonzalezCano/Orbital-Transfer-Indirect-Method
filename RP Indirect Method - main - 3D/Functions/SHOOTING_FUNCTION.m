...
function [ER, yout_all, tout_all] = SHOOTING_FUNCTION(YP, pData, save_data)
    % This function has the logic for simulating and updating ER for each arc
    ...
    % - Update ER based on boundary conditions 
    ...
    for i_arc = 1:pData.N_arcs
        ...
        if i_arc == 1
            y0 = [1; 0; 0; 0; 1; 0; 1; YP(pData.N_arcs + 1); YP(pData.N_arcs + 3); ...
                YP(pData.N_arcs + 4); YP(pData.N_arcs + 5); YP(pData.N_arcs + 6);...
                YP(pData.N_arcs + 7)];
        else
            y0 = yb(i_arc - 1, :);
        end
        ...
        [tout, yout] = ode113(@(t, y) MyOde(t, y, YP, thrust, i_arc, pData), tspan, y0, options);
