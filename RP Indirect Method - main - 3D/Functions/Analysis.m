...
function [DV_total, Time] = Analysis(yout_opt, YP_opt, pData)
    ...
    for i = 1:2:pData.N_arcs % Iterate through burn arcs
        u_0 = yout_opt{i}(4, 1);
        u_f = yout_opt{i}(4, end);
        Delta_u = abs(u_f - u_0);

        v_0 = yout_opt{i}(5, 1);
        v_f = yout_opt{i}(5, end);
        Delta_v = abs(v_f - v_0);

        w_0 = yout_opt{i}(6, 1);
        w_f = yout_opt{i}(6, end);
        Delta_w = abs(w_f - w_0);

        DV(i) = sqrt(Delta_u^2 + Delta_v^2 + Delta_w^2);
    end
    
    DV_total = sum(DV);
    
    fprintf('Total Delta V of the transfer: %.4f adimensional speed units \n' ...
        , DV_total);
