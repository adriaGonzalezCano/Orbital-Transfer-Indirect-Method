...
function Display(ER_data, YP_opt, yout_opt, tout_opt, iter, pData)
    ...
    %% 3. Display - variables
    variable_names = {'r', '\theta', 'u', 'v', 'm', '\lambda_r', '\lambda_u', '\lambda_v', '\lambda_m'};
    ...
    %% Display thrust and trajectory angles
    % Initialize thrust_angle and velocity_angle arrays
    thrust_angle = cell(pData.N_arcs, 1);
    velocity_angle = cell(pData.N_arcs, 1);
    
    % Calculate thrust angles and velocity angles for each arc
    for i_arc = 1:pData.N_arcs
        % Calculate velocity angle for all arcs
        u = yout_opt{i_arc}(:, 3);  % Horizontal velocity in the 3rd column
        v = yout_opt{i_arc}(:, 4);  % Vertical velocity in the 4th column
        velocity_angle{i_arc} = pi/2-atan2(v, u);

        if mod(i_arc, 2) == 1  % Check if it's a burn arc
            % Calculate thrust angle for burn arcs
            thrust_angle{i_arc} = asin(yout_opt{i_arc}(:, 7) ./ sqrt(yout_opt{i_arc}(:, 7).^2 + ...
                yout_opt{i_arc}(:, 8).^2));
        else
            % For coast arcs, set thrust angle to zero
            thrust_angle{i_arc} = zeros(size(tout_opt{i_arc}));
        end
    end
    ...
    for i_arc = 1:pData.N_arcs
        if mod(i_arc, 2) == 1  % Plot only for burn arcs
            if i_arc == 1
                time = tout_opt{i_arc} * YP_opt(i_arc);
            else
                time = (tout_opt{i_arc} - (i_arc - 1)) * YP_opt(i_arc) + cumulative_time(i_arc-1);
            end
            h = plot(time, rad2deg(thrust_angle{i_arc}), 'r', 'LineWidth', 0.5);
            if isempty(thrust_legend)
                thrust_legend = h;
            end
        end
    end

    % Plot velocity angles
    for i_arc = 1:pData.N_arcs
        if i_arc == 1
            time = tout_opt{i_arc} * YP_opt(i_arc);
        else
        ...
        h = plot(time, rad2deg(velocity_angle{i_arc}), 'b-.', 'LineWidth', 0.5);
        if isempty(velocity_legend)
            velocity_legend = h;
        end
    end
    ...
    xlim([0, YP_opt(pData.N_arcs) + sum(YP_opt(1:pData.N_arcs-1))]);
    title('Angle\ vs.\ Time', 'Interpreter', 'latex');

    % Update legend
    legend([thrust_legend, velocity_legend], 'Thrust Angle', 'Velocity Angle', 'Interpreter', 'latex');
